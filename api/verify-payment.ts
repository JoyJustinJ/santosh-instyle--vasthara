...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (token === 'ADMIN_BYPASS' && req.headers.host?.includes('localhost')) {
    ...
    try {
      const payload = token.replace('PHONEAUTH_', '');
      const firstUnderscore = payload.indexOf('_');
      const userPhone = payload.substring(0, firstUnderscore);
      const userPin = payload.substring(firstUnderscore + 1);
    ...
    if (purpose === 'emi' || purpose === 'subscription') {
      ...
      for (const { ref: schemeRef, current, accountId } of validAccounts) {
        ...
        batch.update(schemeRef, {
          ...
          // Only write completedAt if actually completing now, or if it was already set.
