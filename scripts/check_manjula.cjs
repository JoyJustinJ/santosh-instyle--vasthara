...
async function main() {
    console.log("=== SEARCHING FOR CUSTOMER 'MANJULA.A' / PHONE '8870384464' IN FIRESTORE ===\n");

    const usersSnap = await db.collection('users').get();
    let matches = [];
    usersSnap.forEach(doc => {
        const u = doc.data();
        if ((u.phone && (u.phone.includes('8870384464') || u.phone.includes('384464'))) || 
            (u.firstName && u.firstName.toLowerCase().includes('manjula')) ||
            (u.name && u.name.toLowerCase().includes('manjula'))) {
    ...
    schemesSnap.forEach(doc => {
        const s = doc.data();
        if ((s.phone && (s.phone.includes('8870384464') || s.phone.includes('384464'))) ||
            (s.userName && s.userName.toLowerCase().includes('manjula')) ||
            matches.some(m => m.id === s.userId || m.phone === s.phone || m.phone === s.userId)) {
            matchingSchemes.push({ docId: doc.id, ...s });
        }
    });

    console.log(`\nFound ${matchingSchemes.length} scheme(s) for Manjula:`, matchingSchemes);
