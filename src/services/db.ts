import { collection, doc, getDocs, getDocsFromServer, setDoc, deleteDoc, addDoc, query, where, getDoc, runTransaction, orderBy, updateDoc, writeBatch } from 'firebase/firestore';
...
export const getUserByPhone = async (phoneOrId: string) => {
    try {
        ...
        const phoneVariants = [
            ...
            phoneOrId.trim(),
            Number(phoneOrId.replace(/\D/g, '')) // Allow Number matching for backend imported users
        ];
...
export const enrollUserInScheme = async (userId: string, scheme: any, staffId: string, referralEmpId?: string) => {
    try {
        ...
        // Count existing schemes for this user with the same amount to get Nth occurrence
        const existingPlansQuery = query(
            collection(db, "user_schemes"),
            where("userId", "==", userId)
        );
        const existingPlansSnap = await getDocs(existingPlansQuery);
        const existingPlans = existingPlansSnap.docs.map(d => d.data());
        const sameAmtCount = existingPlans.filter(
            p => Number(p.amount || p.monthlyAmount || 0) === amount
        ).length;
        const nth = String(sameAmtCount + 1).padStart(2, '0');

        // Format: DDMMYYYY + Amount + Nth (e.g., 0207202650001)
