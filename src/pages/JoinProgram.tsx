...
const JoinProgram = () => {
  ...
  const { JoinProgram } = usePrograms() as any;
  ...
  const handleJoinAfterPayment = async () => {
    ...
    try {
      ...
      const payment = await payWithRazorpay({
        ...
        receipt: `join_${Date.now()}_${Math.floor(Math.random() * 1000)}`,
      ...
      const account = await JoinProgram(scheme, planId, userId, accountId, {
      ...
      console.error(err);
      const errorMessage = err instanceof Error ? err.message : "Failed to join program. Please try again.";
  ...
  if (step === 'payment') {
    return (
      ...
      >
        ...
        <Card className="bg-surface p-8 border border-border shadow-subtle rounded-3xl w-full">
        ...
        <Card className="p-0 overflow-hidden border-2 border-accent/20">
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="flex items-center gap-3 p-3 bg-accent-light rounded-xl text-accent">
              <ShieldCheck size={18} />
              <p className="text-[10px] font-bold uppercase tracking-wider">Secured by Vastra Trust</p>
            </div>
          </div>
        </Card>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            <Input
              ...
              placeholder="Staff Employee ID (e.g. 102)"
              value={referralCode}
          ...
          >
            ...
            >
              ...
              <div className="p-6 overflow-y-auto text-sm text-text-secondary space-y-4 flex-1">
                ...
                <p><strong>4. Redemption:</strong> Membership rewards must be redeemed for apparel/products at the Santhosh Silks store in Hosur. Cash refunds are not permitted under any circumstances.</p>
