...
type ViewState = 'overview' | 'create_scheme' | 'manage_schemes' | 'deposit' | 'transactions' | 'staff' | 'staff_mgmt' | 'customer_update' | 'customer_report' | 'settings' | 'analytics' | 'defaulters' | 'broadcast' | 'audit_logs' | 'redemptions' | 'create_customer' | 'enroll_customer';
...
const AdminDashboard = () => {
    ...
    const handleUpdatePhone = async () => {
        ...
        try {
            ...
            if (res.success) {
                ...
                showNotif("OTP sent to new phone number", 'info');
            } else {
    ...
    const handleInitiateAdminPreClose = async (plan: any) => {
        try {
            ...
            if (result.success) {
                ...
                showNotif(`OTP sent to ${reportCustomer.phone} for pre-close verification.`, 'info');
            } else {
    ...
    const handleExportCustomers = () => {
        ...
        const rows = plansWithUsers.map(({ plan, user }) => {
            ...
            // Calculate Nth scheme of this amount for this user
            const userPlans = allPlans.filter(p => p.userId === plan.userId);
            ...
            sameAmtPlans.sort((a, b) => new Date(a.enrollmentDate || a.createdAt).getTime() - new Date(b.enrollmentDate || b.createdAt).getTime());
            const planIndex = sameAmtPlans.findIndex(p => p.id === plan.id);
            const nth = String((planIndex >= 0 ? planIndex : 0) + 1).padStart(2, '0');

            // Get DoJ as DDMMYYYY
            let d = new Date(plan.enrollmentDate || plan.createdAt || new Date());
            
            // Check if enrollmentDate is DD-MM-YYYY
            if (plan.enrollmentDate) {
                const parts = plan.enrollmentDate.split(/[-/]/);
                if (parts.length === 3 && parts[2].length === 4) {
                    const testD = new Date(plan.enrollmentDate);
                    if (isNaN(testD.getTime()) || plan.enrollmentDate.indexOf('-') !== -1) {
