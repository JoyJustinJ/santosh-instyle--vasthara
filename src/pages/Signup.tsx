...
const Signup = () => {
  ...
  const { signupWithEmail, sendVerificationEmail, setUser } = useAuth()!;
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.verifyMethod === 'email') {
        ...
        // Real OTP sending via Pay4SMS or skip if already verified
        if (navState?.phoneNumber && formData.phone === navState.phoneNumber) {
          // Phone was already verified on the Login page
          ...
          const finalProfile = {
            ...
            emailVerified: false,
            phoneVerified: true,
            accountCreatedVia: 'phone',
            createdAt: new Date().toISOString(),
          };
          
          await createUserProfile(formData.phone, finalProfile);
          
          const saved = await getUserByPhone(formData.phone);
          if (saved) {
          ...
          setLoading(false);
          setTimeout(() => navigate('/set-pin'), 1500);
        } else {
      ...
      <form onSubmit={handleSignup} className="space-y-6">
        <Card className="p-6 space-y-6 border-none shadow-card">
          ...
          <div className="pt-4 border-t border-border/50">
            <Input
              label="Referral Staff Mobile (Optional)"
              placeholder="Enter staff 10-digit mobile number"
              icon={Smartphone}
              type="tel"
              maxLength={10}
              value={formData.referralStaff}
              onChange={(e) => setFormData({ ...formData, referralStaff: e.target.value.replace(/\D/g, '') })}
            />
            <p className="text-[10px] text-text-muted mt-2 ml-1 italic">
              If a staff member helped you sign up, enter their 10-digit mobile number here for their incentive.
            </p>
          </div>
