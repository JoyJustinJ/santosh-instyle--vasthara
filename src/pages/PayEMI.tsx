...
const PayEMI = () => {
  ...
  const { userSchemes, payEMI } = usePrograms() as any;
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const payment = await payWithRazorpay({
        amount: totalAmount,
        receipt: `emi_${selectedPlans[0]}_${Date.now()}`.substring(0, 40),
      ...
      showNotification(t('pay_emi.payment_success_msg'), "success");
      setSuccess(true);
    } catch (err) {
      console.error(err);
      showNotification(err instanceof Error ? err.message : t('pay_emi.payment_failed_msg'), "error");
    } finally {
      setLoading(false);
    }
  };
      ...
      >
        ...
        <div className="space-y-2">
          <h2 className="text-3xl font-display font-bold text-primary tracking-tight">
            {t('pay_emi.payment_successful')}
          </h2>
          <p className="text-sm font-medium text-text-secondary">
            {t('pay_emi.payment_recorded')}
          </p>
        </div>

        <Card className="w-full bg-surface border-none p-6 space-y-4 text-left">
          <div className="flex justify-between items-center border-b border-border pb-3">
            <p className="text-[10px] font-black text-text-muted uppercase tracking-widest">{t('pay_emi.transaction_id')}</p>
            <p className="text-[10px] font-bold text-text-muted font-mono">{realTransactionIds[0] || '—'}</p>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <p className="text-[10px] font-black text-text-muted uppercase tracking-wider">{t('pay_emi.amount_paid')}</p>
              <p className="text-lg font-bold text-primary">{formatCurrency(totalAmount)}</p>
            </div>
            <div>
              <p className="text-[10px] font-black text-text-muted uppercase tracking-wider">{t('pay_emi.subscriptions_updated')}</p>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-display font-bold text-primary tracking-tight">{t('pay_emi.title')}</h1>
      </div>
      ...
      <div className="space-y-8">
                  ...
                  <div className="flex justify-between items-center">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {isPaid && <p className="text-[10px] text-success font-bold uppercase tracking-wider">{t('pay_emi.already_paid')}</p>}
                      </div>
                    </div>
                    <div className="text-right">
                      ...
                      )}>{isPaid ? t('pay_emi.paid') : t('pay_emi.due')}</p>
                    </div>
                  </div>
                </Card>
              );
        ...
        <div className="space-y-4">
          <h3 className="text-xs font-black text-text-muted uppercase tracking-[0.2em] ml-2">{t('pay_emi.payment_method')}</h3>
          <div className="space-y-3">
            {[
              { id: 'razorpay', label: t('pay_emi.razorpay_checkout'), icon: CreditCard }
            ].map((method) => (
              <button
                key={method.id}
        ...
        <div className="bg-surface rounded-2xl p-6 space-y-4">
          <div className="flex justify-between items-center">
            <p className="text-sm font-medium text-text-secondary">{t('pay_emi.total_amount')}</p>
      ...
      <div className="mt-auto pt-4">
        ...
        >
          {t('pay_emi.proceed_to_pay')} {formatCurrency(totalAmount)}
        </Button>
      </div>
    </motion.div>
  );
